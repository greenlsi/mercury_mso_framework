...
from mercury.msg.network import EnableChannels
from mercury.msg.network.packet.app_layer.ran import ConnectRequest, ConnectResponse, RadioResourceControl, \
    DisconnectRequest, DisconnectResponse, CreatePathRequest, RemovePathRequest, SwitchPathRequest, \
    CreatePathResponse, RemovePathResponse, SwitchPathResponse, StartHandOverRequest, StartHandOverResponse, \
    HandOverStarted, HandOverReady, HandOverFinished, HandOverResponse
from ...common import ExtendedAtomic


class AccessAndControl(ExtendedAtomic):
    ...
    def __init__(self, ap_id: str):
        ...
        """
        super().__init__('aps_ap_{}_access_control'.format(ap_id))

        self.simple = RadioAccessNetworkConfig.bypass_amf       # Simple A&C bypasses AMF
    ...
    def _process_create_path_response(self, job: CreatePathResponse, overhead: str):
        ...
        self.add_msg_to_queue(self.output_access_response, ConnectResponse(self.ap_id, ue_id, response))
    ...
    def _process_remove_path_response(self, job: RemovePathResponse, overhead: str):
        ...
        if response:
            ...
            self.ue_path[ue_id] = self.UE_CONNECTED
        msg = DisconnectResponse(self.ap_id, ue_id, response)
    ...
    def _process_switch_path_response(self, job: SwitchPathResponse, overhead: str):
        ...
        if response:
            self.ue_path[ue_id] = self.UE_CONNECTED
            self._send_connected_ue_list()
        msg = HandOverFinished(self.ap_id, ue_id, self.ue_to_ho_from[ue_id], response)
        self.add_msg_to_queue(self.output_ho_finished, msg)

    def _process_access_request(self, job: ConnectRequest, overhead: str):
        ...
        if ue_id in self.ue_path:
            if self.ue_path[ue_id] == self.UE_CONNECTED:
                msg = ConnectResponse(self.ap_id, ue_id, True)
        ...
        # (if simple) Else, if UE is not connected, send affirmative connection response
            ...
            self.ue_path[ue_id] = self.UE_CONNECTED
            self._send_connected_ue_list()
            self.add_msg_to_queue(self.output_access_response, ConnectResponse(self.ap_id, ue_id, True))
        # (if complex) Else, if UE is not connected, send connection request to AMF
        else:
            self.ue_path[ue_id] = self.UE_CONNECTING
            msg = CreatePathRequest(self.ap_id, ue_id)
    ...
    def _process_disconnect_request(self, job: DisconnectRequest, overhead: str):
        ...
        if ue_id not in self.ue_path:
            ...
            msg = DisconnectResponse(self.ap_id, ue_id, True)
            self.add_msg_to_queue(self.output_disconnect_response, msg)
            ...
            if self.simple:
                ...
                msg = RemovePathRequest(self.ap_id, ue_id)
    ...
    def _process_rrc_msg(self, job: RadioResourceControl, overhead: str):
        ...
        if ue_id not in self.ue_path:
            ...
            if best_ap_id != self.ap_id:
                ...
                msg = StartHandOverRequest(self.ap_id, best_ap_id, ue_id)
    ...
    def _process_start_ho_request(self, job: StartHandOverRequest, overhead: str):
        ...
        msg = StartHandOverResponse(self.ap_id, ap_from, ue_id, True)
    ...
    def _process_start_ho_response(self, job: StartHandOverResponse, overhead: str):
        ...
        if response:
            msg = HandOverStarted(self.ap_id, ue_id, new_ap_id)
    ...
    def _process_ue_ready(self, msg: HandOverReady, overhead: str):
        ...
        if self.simple:
            self.ue_path[ue_id] = self.UE_CONNECTED
            self._send_connected_ue_list()
            msg = HandOverFinished(self.ap_id, ue_id, self.ue_to_ho_from[ue_id], True)
            self.add_msg_to_queue(self.output_ho_finished, msg)
        else:
            self.ue_path[ue_id] = self.UE_HANDOVER_READY
            msg = SwitchPathRequest(self.ap_id, ue_id, self.ue_to_ho_from[ue_id])
    ...
    def _send_connected_ue_list(self):
        ...
        self.add_msg_to_queue(self.output_connected_ue_list, EnableChannels(self.ap_id, set(ue_list)))
